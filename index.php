...
function bluet_kw_load_scripts_front() {
	...
	if($opt_tmp['bt_kw_alt_img']=='on'){
		/*lanch only if needed in the admin pages*/
		wp_enqueue_script( 'functions-script', plugins_url('assets/functions_alt_img_tooltip.js',__FILE__), array(), false, true );
		/**/
	}
}
				...
				if(!empty($my_keywords_ids)){
					...
					wp_reset_postdata();
						...
						// first preg replace to eliminate html tags 						
							...
							$cont=preg_replace('#('.$regex.')#i','**TAG**',$cont); //replace tags by **TAG**							
						//end
						...
						foreach($my_keywords_terms as $id=>$arr){
							...
							$cont=preg_replace('#(__('.$term.')__)#i',$delimiter_1.''.$img.''.$delimiter_2.''.$dfn.''.$delimiter_3,$cont,$limit_match);
							$cont=preg_replace('(<img)','<img height="auto !important"',$cont);
						}
						
						//Reinsert tag HTML elements
						foreach($out[0] as $id=>$tag){						
							$cont=preg_replace('#(\*\*TAG\*\*)#',$tag,$cont,1);
						}
