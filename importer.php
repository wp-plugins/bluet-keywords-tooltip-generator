...
?>
	<form  action="<?php echo(get_admin_url().'tools.php?page=my_keywords_settings_importer'); ?>" method="post">
		<h2>Begin converting my keywors type.</h2>
		...
		?>
		Import from : <select name="bluet_posttypes_list">
		<?php 
		...
		if ( $the_query->have_posts() ) {
			...
			?>
				<div id="bluet_convertion_render">
					<h3><span style="color:green;"><?php echo($nbr_posts_converted); ?></span> keywords converted !</h3>
					<?php
						if(count($duplicated_titles)>0){
					?>
					<h3>These posts were not imported (Duplicated) :</h3>
					<ul style="list-style: initial; padding-left: 25px;">
					<?php foreach($duplicated_titles as $title){ ?>
						<li><b><?php echo($title); ?></b></li>
						<?php } ?>
					</ul>
					<p style="color:green;">if you want to re-import them, you have to delete them from My Keywords.</p>
					<?php
					}
					?>
				</div>
